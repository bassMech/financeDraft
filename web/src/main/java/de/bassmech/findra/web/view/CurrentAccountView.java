...
public class CurrentAccountView extends AccountViewBase {
	...
	private void preFillMaps() {
		for (int i = 1; i <= 12; i++) {
			selectableMonths.put(i,
					java.time.Month.of(i).getDisplayName(TextStyle.FULL_STANDALONE, Locale.getDefault()));
		}
		...
		for (TransactionGrouping grouping : TransactionGrouping.values()) {
			selectableTransactionGroupingDisplayOptions.put(grouping.getDbValue(),
					LocalizationUtil.getTag(grouping.getTagKey()));
		}

		selectableExpectedDay = new LinkedHashMap<>();
		selectableExpectedDay.put(ExpectedDay.UNKNOWN.getDbValue(),
				LocalizationUtil.getTag(ExpectedDay.UNKNOWN.getTagString()));
		selectableExpectedDay.put(ExpectedDay.ULTIMO.getDbValue(),
				LocalizationUtil.getTag(ExpectedDay.ULTIMO.getTagString()));
	...
	public String getTransactionRowColorString(TransactionBaseViewModel vm) {
		if (BigDecimal.ZERO.compareTo(vm.getValue()) > 0) {
			return vm instanceof DraftViewModel || ((TransactionViewModel) vm).getExecutedAt() == null
					? CssReference.TRANSCTION_BG_NEGATIVE_EXPECTED.getValue()
					: CssReference.TRANSCTION_BG_NEGATIVE_EXECUTED.getValue();
		} else if (BigDecimal.ZERO.compareTo(vm.getValue()) < 0) {
			return vm instanceof DraftViewModel || ((TransactionViewModel) vm).getExecutedAt() == null
					? CssReference.TRANSCTION_BG_POSITIVE_EXPECTED.getValue()
					: CssReference.TRANSCTION_BG_POSITIVE_EXECUTED.getValue();
		} else {
			return null;
		}
	}
	...
	public void onAccountChanged() {
		...
		selectedAccount = selectableAccounts.stream().filter(acc -> acc.getId().equals(selectedAccountId)).findFirst()
				.orElse(null);
	...
	public void openAccountDetailDialogEdit() {
		...
		accountDialog.setSelectedDisplayOptionTransactionColumnLayout(
				selectedAccount.getDisplayOptionTransactionColumnLayout().getDbValue());
		accountDialog.setSelectedDisplayOptionTransactionGrouping(
				selectedAccount.getDisplayOptionTransactionGrouping().getDbValue());
	...
	private boolean isAccountDialogValid() {
		...
		if (accountDialog.getStartingYear() < yearRange[0] || accountDialog.getStartingYear() > yearRange[1]) {
			FacesMessageHandler.addMessageFromKey(FacesMessage.SEVERITY_ERROR, "error.starting.year.must.be.in.range",
	...
	public void closeDialogAndSaveAccount() {
		...
		if (isAccountDialogValid()) {
			...
			reloadSelectableAccounts();
			selectedAccount = selectableAccounts.stream().filter(acc -> acc.getId().equals(newAccount.getId()))
					.findFirst().orElse(null);
			selectedAccountId = selectedAccount.getId();
			init();

			transactionColumnLayout = selectedAccount.getDisplayOptionTransactionColumnLayout();
			transactionGrouping = selectedAccount.getDisplayOptionTransactionGrouping();
...
///
	...
	public void onDraftNew() {
		...
		AccountingMonthViewModel monthVm = yearVm == null ? null
				: yearVm.getMonths().stream().filter(month -> month.getMonth() == selectedMonth).findFirst()
						.orElse(null);
	...
	private void prepareAndOpenTransactionDetailDialogForDraft(Integer draftId) {
		TransactionDraftDetailDialogViewModel transactionDialog = new TransactionDraftDetailDialogViewModel(
				LocalizationUtil.getTag(draftId == null ? TagName.DRAFT_NEW.getValue() : TagName.DRAFT_EDIT.getValue()),
				true);
		...
		// transactionDialog.setSelectableExpectedDay(selectableExpectedDay.sub
		// (transactionId));
...
///
	...
	private boolean isTransactionDialogValid() {
		...
		if (transactionDialog.isDraft()) {
			...
			if (draftDialog.getSelectedEndMonth() != null && draftDialog.getSelectedEndYear() != null) {
				YearMonth startYear = YearMonth.of(draftDialog.getSelectedStartYear(),
						draftDialog.getSelectedStartMonth());
	...
	private void prepareAndOpenTransactionDetailDialogForTransaction(Integer transactionId,
			AccountingMonthViewModel accountingMonth) {
		TransactionDetailDialogViewModel transactionDialog = new TransactionDetailDialogViewModel(
				LocalizationUtil.getTag(transactionId == null ? TagName.TRANSACTION_NEW.getValue()
						: TagName.TRANSACTION_EDIT.getValue()),
				false);
		...
		if (transactionId == null) {
			...
			TransactionBaseViewModel vmb = accountingMonth.getTransactions().stream()
					.filter(tr -> !tr.isDraft() && tr.getId().equals(transactionId)).findFirst().orElse(null);
			TransactionViewModel vm = (TransactionViewModel) vmb;
			transactionDialog.setId(vm.getId());
			transactionDialog.setAccountId(selectedAccountId);
			transactionDialog.setExecutedAt(
					vm.getExecutedAt() == null ? null : vm.getExecutedAt().atZone(ZoneOffset.UTC).toLocalDate());
			transactionDialog.setExpectedDay(vm.getExpectedDay());
			transactionDialog.setTitle(vm.getTitle());
			transactionDialog.setDescription(vm.getDescription());
			transactionDialog.setValue(vm.getValue());
	...
	///
	public void onTransactionBaseExecuted(int transactionId, boolean isDraft,
			AccountingMonthViewModel accountingMonth) {
	...
	public boolean isTransactionBaseExecutedDialogValid() {
		...
		if (transactionExecutedDialog.getExecutedAt() == null) {
			FacesMessageHandler.addMessageFromKey(FacesMessage.SEVERITY_ERROR, "error.date.must.not.be.empty");
			isValid = false;
		}
	...
	public void closeDialogAndSaveTransactionBaseExecutedAt() {
		...
		if (isTransactionBaseExecutedDialogValid()) {
			...
			TransactionBaseViewModel vmb = transactionExecutedDialog.getAccountingMonth().getTransactions().stream()
					.filter(tr -> tr.getId().equals(transactionExecutedDialog.getId())).findFirst().orElse(null);
	...
	public String getCurrencyFormatted(BigDecimal value) {
		return FormatterUtil.getCurrencyNumberFormattedWithSymbol(value, settingService.getDbNumberGrouping(client),
				settingService.getDbCurrency(client).getSymbol(),
				settingService.getDbCurrencySymbolPosition(client) == "p");
	}

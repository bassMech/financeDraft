...
import de.bassmech.findra.web.util.statics.enums.ClientFetchErrorCode;
...
@Component
@ViewScoped
public class AccountRecoveryView {
	private String name;
		...
		if (isInputValid()) {
			...
			if (!client.getRecoveryCode().equals(recoveryCode)) {
				...
				throw new ClientFetchException(
						String.format(ClientFetchErrorCode.CLIENT_003.getLoggerMessage(), recoveryCode));
			}
